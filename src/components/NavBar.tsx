...
export default function NavBar(): JSX.Element {
  ...
  return (
    <div className="fixed bottom-16 md:w-[105%] w-[90%] border-2 py-1 px-2 m-0  rounded-[54.88px] h-[94.97px] z-50 bg-[#b7b0b0] bg-opacity-80 text-white border-1 border-[#DFDADA17] md:sticky md:bottom-5 md:left-2 md:h-[50px] ">
      <div className="h-full w-full m-0 pt-2 flex justify-around items-center ">
        {navItems.map((navItem, index) => (
          <button key={index} className="flex flex-col items-center gap-0">
            <div className="h-10 w-[30px] md:h-7 md:w-5 transition-all duration-300 ease-in-out hover:scale-110">
              <Image
                src={navItem.imgSrc}
                alt={navItem.name}
                width={32}
